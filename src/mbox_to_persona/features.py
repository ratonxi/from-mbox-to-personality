...
def is_automated_row(row: dict) -> bool:
    subject = (row.get("subject") or "").lower()
    excerpt = (row.get("redacted_excerpt") or "").lower()
    sender = (row.get("from") or "").lower()
    markers = [
        "automatically generated by gmail",
        "weekly wpforms summary",
        "monsterinsights summary",
        "no-reply",
        "noreply",
        "mailer-daemon",
        "delivery status notification",
    ]
    return any(marker in subject or marker in excerpt or marker in sender for marker in markers)
